...
# # --- Configuration ---
# MODEL_DIR = './saved_model2/' # Your ML model directory
# # --- Load necessary ML objects (Keep this section as is) ---
# print("--- Loading ML Prediction Components ---")
# try:
#     # Load your ML model, scaler, encoders, feature columns, evidence_info etc.
#     # This part remains identical to your previous working API script
...
#     # --- Load FULL evidence data needed for Gemini prompt context ---
#     # NOTE: Sending all this data in the prompt might be too large.
#     # Consider creating a smaller summary or using RAG in a more advanced setup.
#     # For now, we load it to potentially extract examples for the prompt.
...
#          print(f"Loaded full evidence data from {evidence_json_path} for prompt context.")
#     else:
#         print(f"*** Warning: Full evidence JSON not found at {evidence_json_path}. Gemini mapping might be less accurate. ***")
#         full_evidence_data = None # Handle gracefully if missing

#     # Load evidence_info generated during training (needed for process_patient_for_prediction_adv)
#     evidence_info_path = os.path.join(MODEL_DIR, 'evidence_info.json');
#     with open(evidence_info_path, 'r') as f: evidence_info = json.load(f); print("Loaded training evidence info.")
...
#     # Use gemini-1.5-flash - check model name if needed:
...
# # --- Pre-process Evidence Info for process_patient_for_prediction_adv (REVISED) ---
# # This part is needed to know if a code is numerical *after* Gemini provides it
# print("Pre-processing training evidence info for prediction function...")
# processed_evidence_for_pred = {}
# numerical_c_codes_for_pred = set()
# for code, details in evidence_info.items(): # Use evidence_info from training
...
#     if data_type == 'C' and possible_values and not any(str(v).startswith('V_') for v in possible_values):
...
# # --- End ML Component Loading ---


# # --- NEW: Function to build the prompt for Gemini ---
# def build_gemini_prompt(user_symptoms_list):
#     """Constructs the prompt for the Gemini API."""

#     # --- Provide Clear Instructions ---
#     instructions = """
#         You are a helpful AI assistant specialized in mapping patient-reported symptoms to specific medical evidence codes. Your task is to analyze the provided list of symptoms and convert each symptom into its corresponding code(s) based on the defined evidence structure.

#         **Output Format Rules (CRITICAL):**
#         1.  You MUST output **only** a valid Python list of strings.
#         2.  Each string in the list must be a valid evidence code from the context provided below.
#         3.  For simple boolean symptoms (like fever), use the format: `"E_code"` (e.g., `"E_91"`).
#         4.  For symptoms specifying a location, characteristic, or category (Multivalue 'M' or Categorical 'C' with V_ codes), use the format: `"E_code_@_V_code"` (e.g., `"E_55_@_V_17"` for 'groin(L)' related to pain).
#         5.  For symptoms providing a numerical value (like a pain scale, data_type 'C' with numerical possible-values), use the format: `"E_code_@_Value"` (e.g., `"E_56_@_5"` for pain intensity 5).
#         6.  If a symptom corresponds to multiple codes, include all relevant codes in the list.
#         7.  If a symptom cannot be confidently mapped to any known code, **do not include any code for it** in the output list. Do not add explanations or apologies.
#         8.  Ensure the final output starts with `[` and ends with `]`. Example: `['E_91', 'E_55_@_V_17', 'E_56_@_5']`

#         **Evidence Context & Examples:**
#         *   **Fever:** Maps to `E_91` (Question: "Do you have a fever...?"). Output: `"E_91"`
#         *   **Pain:** General pain questions exist.
#             *   Location: `E_55` (Question: "Do you feel pain somewhere?") uses `V_code` for location. Example: "left groin pain" should map to `"E_55_@_V_17"` because `V_17` means `groin(L)`. "right ankle pain" -> `"E_55_@_V_34"`.
#             *   Intensity: `E_56` (Question: "How intense is the pain?") takes a numerical value 0-10. Example: "pain intensity 7" or "pain 7/10" should map to `"E_56_@_7"`.
#         *   **Lesion:** General lesion question `E_129` exists.
#             *   Location: `E_133` (Question: "Where is the affected region located?") uses `V_code` for location. Example: "lesion on left forearm" -> `"E_133_@_V_28"`.
#             *   Pain Intensity: `E_134` uses numerical scale 0-10. Example: "lesion pain 4" -> `"E_134_@_4"`.
#         *   **Swelling:** General swelling question `E_151` exists.
#             *   Location: `E_152` (Question: "Where is the swelling located?") uses `V_code` for location. Example: "swelling in right ankle" -> `"E_152_@_V_34"`.
#         *   **Cough:** Maps to `E_201` (Question: "Do you have a cough?"). Output: `"E_201"`
#         *   **Sore Throat:** Maps to `E_97` (Question: "Do you have a sore throat?"). Output: `"E_97"`
#         *   **Difficulty Breathing:** Maps to `E_66` (Question: "Are you experiencing shortness of breath...?"). Output: `"E_66"`
#         *   **Nausea:** Maps to `E_148` (Question: "Are you feeling nauseous...?"). Output: `"E_148"`
#         *   **Vomiting:** Can map to `E_148` (if nausea present) or `E_211` ("Have you vomited several times...?"). If just "vomiting", map to both: `["E_148", "E_211"]`
#         *   **Chills:** Maps to `E_94` (Question: "Have you had chills...?"). Output: `"E_94"`
#         *   **Dizzy / Lightheaded:** Maps to `E_76` (Question: "Do you feel slightly dizzy...?"). Output: `"E_76"`

#         **(You have access to the full evidence structure conceptually, focus on applying these patterns).**

#         **User Symptoms to Map:**
#     """

#     # --- Format the User Symptom List ---
...
# # --- NEW: Function to call Gemini and parse result ---
...
#         return [], [] # No symptoms to map

#     prompt = build_gemini_prompt(user_symptoms_list)
#     print("\n--- Calling Gemini for Symptom Mapping ---")
#     # print(f"Prompt being sent to Gemini:\n{prompt}") # DEBUG: Uncomment to see the full prompt

#     try:
#         # Use safety_settings to reduce refusals for medical content, adjust if needed
#         # Use generation_config to potentially guide output, though prompt is primary
...
#             # ),
#              safety_settings=[ # Allow potentially sensitive medical terms
...
#         # Attempt to parse the output string as a Python list
#         try:
#             # Use ast.literal_eval for safety instead of eval()
#             parsed_list = ast.literal_eval(raw_output)
#             if isinstance(parsed_list, list) and all(isinstance(item, str) for item in parsed_list):
#                  # Basic validation of code format (optional but recommended)
#                  validated_list = []
#                  malformed_codes = []
#                  for item in parsed_list:
#                      if re.match(r'^E_\d+(_@_.*)?$', item): # Matches E_123 or E_123@_V_45 or E_123@_5
#                          validated_list.append(item)
#                      else:
#                          malformed_codes.append(item)
#                  if malformed_codes:
#                       print(f"Warning: Gemini produced malformed codes: {malformed_codes}")
#                  print(f"Successfully Parsed & Validated Gemini Output: {validated_list}")
#                  # Determine unmatched symptoms (simple approach: compare input count to output code count related to unique symptoms)
#                  # This is an approximation as one symptom might map to multiple codes
#                  unmatched_symptoms = [] # Simplified - Can't reliably determine unmatched from LLM output alone
#                  return validated_list, unmatched_symptoms
#             else:
#                 print("Error: Gemini output was not a valid list of strings.")
#                 return None, ["LLM output format error (not list of strings)"]
#         except (ValueError, SyntaxError) as parse_error:
#             print(f"Error parsing Gemini output as Python list: {parse_error}")
...
# # --- process_patient_for_prediction_adv (Slight Modification) ---
# # Renamed processed_evidence -> processed_evidence_for_pred to avoid conflict
...
#     # Ensure all expected columns exist, initialized to 0.0
#     patient_df = pd.DataFrame(0.0, index=[0], columns=all_feature_columns, dtype=np.float32)

#     # Process evidence codes provided (NOW potentially from LLM)
#     for item in evidence_code_value_list:
#         feature_col_name = None
#         value_to_set = 1.0  # Default for Binary or Categorical _@_

#         if '_@_' in item:
#             parts = item.split('_@_', 1) # Split only once
#             base_code = parts[0]
#             value_str = parts[1]

#             # Check if this code was identified as numerical during PRE-PROCESSING
#             if base_code in processed_evidence_for_pred and processed_evidence_for_pred[base_code]['is_numerical_c']:
#                 try:
#                     num_val = float(value_str)
#                     feature_col_name = base_code # Feature is the base code for numerical C
#                     value_to_set = num_val
#                     # print(f"Mapping numerical: {item} -> {feature_col_name} = {value_to_set}") # Debug
#                 except ValueError:
#                     print(f"Warning: Could not convert value '{value_str}' to float for numerical code '{base_code}'. Treating as categorical.")
#                     # Fallback: Treat as categorical feature if conversion fails
#                     feature_col_name = item # Feature includes the value part
#                     value_to_set = 1.0
#             else:
#                 # Categorical _@_ (like E_55@_V_17)
#                 feature_col_name = item # Feature name includes the value part
#                 value_to_set = 1.0
#                 # print(f"Mapping categorical: {item} -> {feature_col_name} = {value_to_set}") # Debug
#         else:
#             # Binary code (like E_91)
#             feature_col_name = item # Feature name is the code itself
#             value_to_set = 1.0
#             # print(f"Mapping binary: {item} -> {feature_col_name} = {value_to_set}") # Debug


#         # Assign value if the column exists in our feature list
...
#     # --- Process AGE and SEX (No change needed here) ---
...
#     # --- Apply Scaling (No change needed here) ---
...
#         # else: print("No columns required scaling based on training info.") # Less verbose
#     elif scaler and not cols_scaled_during_fit:
#         print("Warning: Scaler loaded but list of scaled columns from training missing.")

#     # --- Final Column Alignment (No change needed here) ---
#     try:
#         # Ensure the DataFrame has exactly the columns expected by the model, in the correct order
...
#     # print(f"Processing time: {time.time() - start_time_proc:.4f} seconds.") # Optional timing
...
# # --- API Endpoint (MODIFIED TO USE GEMINI) ---
...
#         # Handle errors from the Gemini mapping step
#         error_detail = ", ".join(mapping_errors) if mapping_errors else "Unknown mapping error"
#         print(f"Symptom mapping failed: {error_detail}")
#         # Decide if you want to expose LLM errors directly or give a generic message
#         return jsonify({"error": f"Symptom mapping failed: {error_detail}"}), 500 # Internal Server Error

#     print(f"Gemini mapped codes: {evidence_codes_and_values}")
#     # Note: Reliable 'unmatched' list is hard with LLM, removing it from response for now.
...
#         # Determine expected features (handle different model types)
...
#         else: model_expected_features = all_feature_columns # Fallback

#         # Align DF columns EXACTLY to model's expectations
...
#         "processed_symptoms_codes": evidence_codes_and_values, # Show the codes used
#         # "unmatched_symptoms": [] # Removing this as it's unreliable with LLM
#     }
...
# def build_gemini_prompt(user_symptoms_list):
#     """Constructs the prompt for the Gemini API, FORCING a mapping attempt for EVERY input item."""

#     instructions = f"""
#         You are a highly diligent AI assistant specialized in mapping patient-reported symptoms OR medical history items to specific medical evidence codes. Your task is to analyze the provided list of {len(user_symptoms_list)} input items (symptoms, history questions, statements) and convert **each and every one** into its corresponding code(s) based on the defined evidence structure.

#         **Output Format Rules (CRITICAL):**
#         1.  You MUST output **only** a valid Python list of strings.
#         2.  **Crucially, you MUST attempt to map EVERY item from the input list.** Do NOT omit any input symptom; find the best possible code match, even if it's a general one. If an input symptom maps to multiple codes (e.g., a symptom and its location), include all. The output list will likely contain at least as many codes as the input symptom list.
#         3.  Each string in the list must be a valid evidence code format based on the context provided below.
...
#         5.  For items specifying a location, characteristic, or category (Multivalue 'M' or Categorical 'C' with V_ codes), use the format: `"E_code_@_V_code"` (e.g., `"E_55_@_V_17"` for 'groin(L)' related to pain).
#         6.  For items providing a numerical value (like a pain scale, data_type 'C' with numerical possible-values), use the format: `"E_code_@_Value"` (e.g., `"E_56_@_5"` for pain intensity 5).
...
#         **Evidence Context & Examples (Includes Natural Language, Structured Input, Questions, and Antecedents):**

#         *Natural Language Examples:*
#         *   "fever" -> `"E_91"`
#         *   "left groin pain" -> `"E_55_@_V_17"` (Maps to specific location)
#         *   "pain intensity 7" -> `"E_56_@_7"`
#         *   "intense headache" -> `"E_55_@_V_89"` (Maps to forehead pain location)
#         *   "headache level 9" -> `["E_55_@_V_89", "E_56_@_9"]` (Maps location and intensity)
#         *   "lesion on left forearm" -> `["E_129", "E_133_@_V_28"]` (Maps general lesion AND specific location)
#         *   "lesion pain 4" -> `"E_134_@_4"` (Maps lesion pain intensity)
#         *   "swelling in right ankle" -> `["E_151", "E_152_@_V_34"]` (Maps general swelling AND specific location)
#         *   "cough" -> `"E_201"`
#         *   "sore throat" -> `"E_97"`
...
#         *   "chills" -> `"E_94"`
#         *   "dizzy" -> `"E_76"`
#         *   "feeling very tired" -> `"E_89"`
#         *   "black stools" -> `"E_140"`
#         *   "underweight" -> `"E_208"`
#         *   "diffuse muscle pain" -> `"E_144"`
#         *   "clear runny nose" -> `"E_181"`
#         *   "I smoke cigarettes" OR "smoker" -> `"E_79"`

#         *Structured Input Examples:*
#         *   "significantly increased sweating" -> `"E_50"`
...
#         *   "Pain radiates to: nowhere" -> `"E_57_@_V_123"`
#         *   "Pain precision 7" -> `"E_58_@_7"`
#         *   "Pain onset speed 5" -> `"E_59_@_5"`
#         *   "lesions/redness/skin problems related to condition" -> `"E_129"`
#         *   "Rash color: pink" -> `"E_130_@_V_156"`
#         *   "Lesions peel off: N" -> `"E_131_@_V_10"`
#         *   "Rash swollen level 0" -> `"E_132_@_0"`
#         *   "Lesion location: side of the neck(G)" -> `"E_133_@_V_54"`
#         *   "Lesion pain intensity 0" -> `"E_134_@_0"`
#         *   "Lesion larger than 1cm: Y" -> `"E_135_@_V_12"`
#         *   "Itching level 1" -> `"E_136_@_1"`
#         *   "loss of appetite / get full quickly" -> `"E_161"`
#         *   "Travel out of country: N" -> `"E_204_@_V_10"`

#         *Question/Antecedent Input Examples:*
#         *   "smoke cigarettes?" -> `"E_79"`
#         *   "any family members asthma?" -> `"E_87"`
#         *   "diagnosed with chronic sinusitis?" -> `"E_119"`
#         *   "history of asthma" -> `"E_124"`
#         *   "asthma / ever had to use bronchodilator in past?" -> `"E_124"`
#         *   "live in big city?" -> `"E_207"`
#         *   "exposed to secondhand cigarette smoke on daily basis?" -> `"E_222"`
#         *   "more likely to develop common allergies than general population?" -> `"E_226"`
#         *   "cold in last 2 weeks?" -> `"E_116"`
#         *   "hospitalized for asthma attack in past year?" -> `"E_101"`

#         *Handling Ambiguity (Default to Boolean Base Code if Specific Value Unknown):*
#         *   "pain in my leg somewhere" -> `"E_53"` (Cannot determine V_code for E_55, default to general pain boolean E_53)
#         *   "some kind of skin rash" -> `"E_129"` (Cannot determine location/color, default to general lesion boolean E_129)
#         *   "pain was pretty bad" -> `"E_53"` (Cannot determine numerical value for E_56, default to general pain boolean E_53)


#         **(Use these patterns diligently. Map EVERY input item to its best possible code(s) according to these rules and examples).**
