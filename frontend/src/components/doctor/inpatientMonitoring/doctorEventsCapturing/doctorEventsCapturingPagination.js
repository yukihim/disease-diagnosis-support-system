...
function DoctorEventsCapturingPagination({ currentPage, totalPages, onPageChange, rowsPerPageOptions, currentRowsPerPage, onRowsPerPageChange }) {
    ...
    return (
        <ComponentPagination>
            ...
            <div style={{
                ...
                <label
                    htmlFor="rowsPerPageSelectEvents" // Use a unique ID
                    style={{
                ...
                <select
                    id="rowsPerPageSelectEvents" // Use a unique ID
                    ...
                    disabled={!options.length || totalPages === 0} // Disable if no options or no data
                >
                    {options.map(option => (
            ...
            <PaginationText>
                ...
                {totalPages > 0 ? `${String(currentPage).padStart(2, '0')}/${String(totalPages).padStart(2, '0')}` : '00/00'}
            </PaginationText>
            <NextButton
                onClick={() => onPageChange(currentPage + 1)}
...
export default DoctorEventsCapturingPagination;
